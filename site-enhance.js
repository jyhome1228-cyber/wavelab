(()=>{
  ...
  const render=async query=>{
    const keyword=query.trim().toLowerCase();if(!keyword){results.innerHTML='<div class="wavelab-search-empty">검색어를 입력하면 관련 콘텐츠가 표시됩니다.</div>';return}
    results.innerHTML='<div class="wavelab-search-empty">콘텐츠를 찾고 있습니다.</div>';const items=await buildIndex();
  ...
  document.addEventListener('error',event=>{
    const image=event.target;if(!(image instanceof HTMLImageElement)||image.dataset.fallbackApplied)return;image.dataset.fallbackApplied='true';
    const parent=image.closest('.article-source-image,.real-thumb');image.removeAttribute('src');image.alt='이미지를 불러오지 못했습니다.';image.style.display='none';
    if(parent&&!parent.querySelector('.image-fallback')){const fallback=document.createElement('div');fallback.className='image-fallback';fallback.innerHTML='<span>●</span><strong>AESOST IMAGE</strong>';parent.prepend(fallback)}
  },true);
})();