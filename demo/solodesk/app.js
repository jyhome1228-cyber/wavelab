const STORAGE_KEY='aesost-solodesk-demo-v1';
const seed={
  clients:[
    {id:'c1',name:'Studio More',email:'hello@studiomore.kr',phone:'010-4821-2301',status:'Active'},
    {id:'c2',name:'Mori Living',email:'brand@moriliving.co.kr',phone:'010-6712-8820',status:'Active'},
    {id:'c3',name:'Arc Health',email:'team@archealth.co.kr',phone:'010-9254-1160',status:'Active'},
    {id:'c4',name:'Atelier Noon',email:'noon@atelier.kr',phone:'010-7310-4428',status:'Lead'}
  ],
  projects:[
...
function setView(name){
  ...
  const titles={dashboard:'Good afternoon, Jaeyoung.',clients:'Client database',projects:'Project workspace',finance:'Finance overview'};
...
function renderAll(){
  renderCounts();renderMetrics();renderDashboardProjects();renderChart();renderUpcoming();renderClients();renderProjects();renderFinance();renderClientSelect();
}
function renderCounts(){
  document.querySelectorAll('[data-count="clients"]').forEach(e=>e.textContent=state.clients.length);
  document.querySelectorAll('[data-count="projects"]').forEach(e=>e.textContent=state.projects.length);
}
...
function renderDashboardProjects(){
  ...
  target.innerHTML=rows.map(p=>`<div class="project-line"><div class="project-main"><b>${esc(p.name)}</b><span>${esc(clientById(p.client)?.name||'Unknown client')}</span></div><span>${won(p.amount)}</span><span>${formatDate(p.due)}</span><span class="status ${statusClass(p.status)}">${p.status}</span></div>`).join('')||'<p>No projects yet.</p>';
}
...
function renderProjects(){
  ...
  target.innerHTML=rows.map(p=>`<article class="project-card"><div class="project-top"><span class="status ${statusClass(p.status)}">${p.status}</span><button class="status-button" data-cycle="${p.id}" title="Change status">•••</button></div><h3>${esc(p.name)}</h3><p>${esc(clientById(p.client)?.name||'Unknown client')}</p><div class="project-meta"><div><span>VALUE</span><b>${won(p.amount)}</b></div><div><span>DUE</span><b>${formatDate(p.due)}</b></div></div></article>`).join('')||'<p>No matching projects.</p>';
...
function renderClientSelect(){
  ...
  select.innerHTML='<option value="">Select client</option>'+state.clients.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');
}
function cycleStatus(id){
  const p=projectById(id);if(!p)return;const order=['In Progress','Review','Complete'];p.status=order[(order.indexOf(p.status)+1)%order.length];saveState();renderAll();toast(`Project moved to ${p.status}`);
}
...
document.querySelector('[data-project-form]').addEventListener('submit',e=>{
  e.preventDefault();const fd=new FormData(e.currentTarget);const project={id:uid('p'),name:fd.get('name').trim(),client:fd.get('client'),amount:Number(fd.get('amount')),status:fd.get('status'),due:fd.get('due'),paid:false};state.projects.unshift(project);state.invoices.unshift({id:'INV-'+new Date().getFullYear().toString().slice(-2)+String(state.invoices.length+1).padStart(3,'0'),project:project.id,amount:Math.round(project.amount*.5),due:project.due,status:'Pending'});saveState();e.currentTarget.reset();closeModals();renderAll();setView('projects');toast('Project created');
});
document.querySelector('[data-client-form]').addEventListener('submit',e=>{
  e.preventDefault();const fd=new FormData(e.currentTarget);state.clients.unshift({id:uid('c'),name:fd.get('name').trim(),email:fd.get('email').trim(),phone:fd.get('phone').trim(),status:'Active'});saveState();e.currentTarget.reset();closeModals();renderAll();toast('Client added');
});
document.querySelector('[data-reset]').addEventListener('click',()=>{if(!confirm('Reset SoloDesk demo data?'))return;state=cloneSeed();saveState();renderAll();toast('Demo data reset')});
