...
const adminHtml=fs.readFileSync(path.join(root,'admin/index.html'),'utf8');if(!adminHtml.includes('admin-current.css'))errors.push('admin/index.html: current admin stylesheet is missing');if(adminHtml.includes('admin.css?')||adminHtml.includes('admin-account.css'))errors.push('admin/index.html: legacy admin styles must not be loaded');if(!adminHtml.includes('aesost-logo-dark.svg'))errors.push('admin/index.html: light admin must use dark AESOST logo');if(!adminHtml.includes('admin-entry.js'))errors.push('admin/index.html: admin entry module is missing');for(const view of ['dashboard','requests','solutions','visitors'])if(!adminHtml.includes(`data-admin-view="${view}"`))errors.push(`admin/index.html: missing ${view} admin view`);checkLocalRefs('admin/index.html',adminHtml);
const adminEntry=fs.readFileSync(path.join(root,'admin/admin-entry.js'),'utf8');for(const module of ['admin-business-dashboard.js','admin-visitor-analytics.js'])if(!adminEntry.includes(module))errors.push(`admin-entry.js: ${module} is not loaded`);
